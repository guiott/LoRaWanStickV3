...
/* All the functions related to display have been removed from this
	lib because they are not compatible with all the displays. 
	They are replaced by the ones in Display.ino 
 ...
 */
RTC_DATA_ATTR int revrssi,revsnr,revDataRate;
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
	...
	revDataRate=mcpsIndication->RxDoneDatarate;
...
// DISPLAY ==================================
// for the Display.ino purposes
bool LoRaWanClass::isJoinedOK()
{
	return joinedOK;
}

bool LoRaWanClass::LoRaWan_ADR()
{
	return loraWanAdr;
}

bool LoRaWanClass::Display_ACK()
{
	return ifDisplayAck;
}

char* LoRaWanClass::dispSignal()
{
	static char temp[11];
    if(ifDisplayAck==0)
    {
    ...
    ifDisplayAck--;
    sprintf(temp,"%d %d",revrssi,revsnr);
	return temp;
}

char* LoRaWanClass::dispDataRate()
{
	static char temp[11];
    sprintf(temp,"DR: %d",revDataRate);
	return temp;
}
